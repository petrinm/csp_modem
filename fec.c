...
int csp_fec_append(csp_packet_t * packet) {
	...
	encode_rs_8(&packet->id, &packet->data[packet->length], CSP_RS_MSGLEN - packet->length);
...
int csp_fec_decode(csp_packet_t * packet) {
	...
	int ret;
	if ((ret = decode_rs_8(&packet->id, NULL, 0, CSP_RS_MSGLEN + CSP_RS_LEN - packet->length)) < 0)
